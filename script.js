(function () {
  ...
  // 既存の「一問一答システム」と同じFirebaseプロジェクトを利用しつつ、
  // Firestoreのコレクション名とAuthのダミーメールドメインを分けることで、
  // 同じユーザーIDでも別アカウントとして扱われ、データが混ざらないようにしている。
  ...
  const USERS_COLLECTION = 'sqliDojoUsers'; // 既存アプリの users コレクションとは別にする
  const LEADERBOARD_COLLECTION = 'sqliDojoLeaderboard'; // ランキング用。username と累計日数だけを持つ(isAdminなどは含めない)
  const EMAIL_DOMAIN = 'sqli-dojo.local'; // 既存アプリのダミードメインとも別にする
